...
class studentplancheck_page extends basePage{
  ...
  function testRange($nbr,$min,$max){
    if ($nbr >= $min && $nbr <= $max){
      return true;
    }//endif
    else {
  ...
  function display_page(){ // Body of page.
    ...
    echo '<td width="10%"><b>Student:</b></td><td>' . $_SESSION["studentplan"]["studentid"] . ' - ' . $othernames . ' ' . $lastname;
    ...
    while (list($key,$value) = each($_SESSION["studentplanunit"])){
      ...
      // check if in term legend array. if not load.
      if (!empty($row["termid"])){
      ...
      if ($uufound){//if found one (anyone) means term has been loaded at location so now look for unit
    ...
    echo '</td><td><table border="0" cellpadding="4" cellspacing="0">';
    echo '<tr>';
    ...
    if (!isset($_POST["hidFirstTime"])){//Enforce validation first time in
